...
class EB_LAMMPS(CMakeMake):
    ...
    def configure_step(self, **kwargs):
        ...
        if self.cfg['kokkos']:
            ...
            karch = get_kokkos_arch(cuda_cc, self.cfg['kokkos_arch'])

            if self.toolchain.options.get('openmp', None):
                if LooseVersion(self.version) >= LooseVersion('29Sep2021'):
            ...
            if LooseVersion(self.version) >= LooseVersion('29Sep2021'):
                self.cfg.update('configopts', '-DKokkos_ARCH_%s=ON' % get_cpu_arch().upper())
            else:
                self.cfg.update('configopts', '-DKOKKOS_ARCH="%s"' % karch)
            ...
            if cuda:
                ...
                if LooseVersion(self.version) >= LooseVersion('29Sep2021'):
                    ...
                    # if cuda, karch does have complete "%s;%s". If not, get_kokkos_arch() would give an error
                    self.cfg.update('configopts', '-DKokkos_ARCH_%s=ON' % karch.split(";")[-1])
                else:
                    self.cfg.update('configopts', '-DKOKKOS_ENABLE_CUDA=yes')
